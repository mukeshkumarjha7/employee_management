...
RSpec.describe "SalaryInsights", type: :request do
  ...
  it "returns bad request when country is not provided" do
    get salary_insights_path, as: :json
    expect(response).to have_http_status(:bad_request)
  end

  # Min, max, avg salary for a country
  it "returns min salary for a country" do
    get salary_insights_path, params: { country: "India" }, as: :json
  ...
  it "returns average salary for a given job title in a country" do
    get salary_insights_path, params: { country: "India", job_title: "Software Engineer" }, as: :json
    expect(response).to have_http_status(:ok)
  ...
  it "returns empty insights for a country with no employees" do
    get salary_insights_path, params: { country: "UK" }, as: :json
    expect(response).to have_http_status(:ok)
  ...
  # Test HTML view
  it "renders the salary insights page" do
    get salary_insights_path
    expect(response).to have_http_status(:ok)
    expect(response.body).to include("Salary Insights")
  end

  it "search form with country input" do
    get salary_insights_path
    expect(response.body).to include('input')
    expect(response.body).to include('country')
  end

  it "salary insights results for a country" do
    get salary_insights_path, params: { country: "India" }
    expect(response).to have_http_status(:ok)
    expect(response.body).to include("50000")
    expect(response.body).to include("80000")
  end

  it "average salary for a job title in given country" do
    get salary_insights_path, params: { country: "India", job_title: "Software Engineer" }
    expect(response).to have_http_status(:ok)
    expect(response.body).to include("57500")
  end

  it "empty result when no employees is found for a country" do
